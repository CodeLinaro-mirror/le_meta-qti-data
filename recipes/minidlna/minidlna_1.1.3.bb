...
do_install:append () {
    ...
    install -m 664  ${UNPACKDIR}/${PN}-${PV}/${PN}.conf ${D}${sysconfdir}/data/
    chown -R root:1001 ${D}${sysconfdir}/data/minidlna.conf
    install -d ${D}${sysconfdir}/init.d/
    install ${UNPACKDIR}/${PN}-${PV}/linux/${PN}.init.d.script ${D}${sysconfdir}/init.d/minidlna
    if ${@bb.utils.contains('DISTRO_FEATURES', 'systemd', 'true', 'false', d)}; then
        rm -rf ${D}${sysconfdir}/init.d/minidlna
        install -d ${D}${systemd_unitdir}/system/
        install -m 0644 ${UNPACKDIR}/minidlnad.service -D ${D}${systemd_unitdir}/system/minidlnad.service
    fi
}
